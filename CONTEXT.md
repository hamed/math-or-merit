...
- **R12 (2026-07-16): the cow got a face.** Hamed's hand-drawn Inkscape cows
  (inbox → `art/cow/`, see its README) replaced the blob cow in `CowScene`.
  Geometry lives in `scenes/cow-geometry.ts` (generated by `art/cow/gen_cow.py`,
  extracted from the sources; head coordinates identical in both poses, so the
  face persists through the morph). Expression arc per Hamed's beat map:
  sad → smile (chemist) → flat (physicist) → eyes-wide (sphere) → gasp/pinched
  nostrils/puffed cheeks/dilated pupils (vacuum) → angry (rolled at the hill)
  → sad again (real/moral). Body morph is an ellipse→circle ATTR tween (not
  MorphSVG); MorphSVG only morphs horn + mouth. `paths.ts` keeps only `SPHERE`
  (PersonScene still morphs into "the same circle the cow became").

163 vitest tests green, `npm run build` clean, headless QA sweeps (desktop /
